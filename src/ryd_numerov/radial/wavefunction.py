...
from ryd_numerov.radial.sanity_check_exceptions import is_known_exception
...
class Wavefunction:
    ...
    def sanity_check(self, z_stop: float, run_backward: bool) -> bool:  # noqa: C901, PLR0915, PLR0912
        ...
        n, l = self.model_potential.n, self.model_potential.l
        ...
        if inner_weight_scaled_to_whole_grid > 1e-4:
            warning_msgs.append(
        ...
        if warning_msgs:
            ...
            msg += "\n      ".join(["", *warning_msgs])
            if not is_known_exception(self.model_potential, warning_msgs):
                logger.error(msg)
            else:
                logger.warning(msg)
            return False
