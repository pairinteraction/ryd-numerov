...
To use custom quantum defects (or quantum defects for a new element), you can simply create a subclass of `ryd_numerov.species.base_element.BaseElement` (e.g. `class CustomRubidium(BaseElement):`) with a custom species name (e.g. `species = "Custom_Rb"`).
